...
def run():
	...
	tre.clock_filter(reroot=args.root, n_iqd=4, plot=False) 
	# actually removes bad tips
	bad_tips = [item.name for item in tre.tree.get_terminals() if 
		item.bad_branch==True and 
		item.name not in args.keep]
	for n in tre.tree.get_terminals():
		if n.bad_branch and n.name not in args.keep:
			tre.tree.prune(n)
